...
const CoursesPageContent = () => {
  ...
  const [allCourses, setAllCourses] = useState(() =>
    generateMockCourses(60, { college: collegeParam, department: departmentParam })
  );

  useEffect(() => {
    setIsLoading(true);
    setAllCourses(generateMockCourses(60, { college: collegeParam, department: departmentParam }));
    setCurrentPage(1);
    const timer = setTimeout(() => {
      setIsLoading(false);
    }, 600);
    return () => clearTimeout(timer);
  }, [typeParam, areaParam, collegeParam, departmentParam]);
        ...
        {Array.from({ length: totalPages }).map((_, i) => {
          ...
          return (
            <button
              ...
              className={`w-10 h-10 flex items-center justify-center rounded-xl text-sm font-semibold transition-all shadow-sm ${isActive ? "bg-[#111] text-white border-transparent" : "bg-white/60 border border-white/50 text-[#111] hover:bg-white/90"}`}
            >
              {page}
            </button>
          );
